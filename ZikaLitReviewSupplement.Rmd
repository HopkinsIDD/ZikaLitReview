...
### 1.1.3 Visualizing the interval-censored data
...
IPplot <- ggplot(dat, aes(y=factor(UID))) + 
    ...
    annotate("text", x=-28, y="900.1", label="Incubation Period")
...
VCplot <- ggplot(dat, aes(y=factor(UID))) + 
    ...
    theme(axis.text.y = element_text(size=8)) +
    coord_cartesian(xlim = c(-40, 50)) +
    annotate("text", x=-28, y="900.1", label="Viral Clearance")
...
ACplot <- ggplot(dat, aes(y=factor(UID))) + 
    ...
    theme(axis.text.y = element_text(size=8)) +
    coord_cartesian(xlim = c(-40, 50)) +
    annotate("text", x=-28, y="900.1", label="Seroconversion")

grid.arrange(IPplot, VCplot, ACplot, ncol=1)
```

**Figure S1:** The intervals observed for each indivdual. Blue intervals represent windows of possible exposure. Red intervals represent windows of time in which the outcome (symptom onset, viral clearance, seroconversion) could have occured.
...
We treat the time of infection for each individual, $E_i$ drawn from a uniform prior defined by the earliest and lates possible times of exposure ($EL_i$ and $ER_i$).
...
We ran three serperate chains for 1,000,000 interations each, thinning to 1 in 50 samples. The first 5,000 of the 20,000 retained samples were discarded as burnin. Convergance was assessed by visual inspection and Gelman and Rubin's $\hat{R}$ statistic [@brooks1998].
...
**Figure S2:** Percent developing symptoms by day t (top), percent with detectable virus in blood at day t (middle), and percent seroconverting by day t (bottom).
...
require(knitr)
inc.fit.jags.LLL <- exp(c(median(chains.LLL$lm), quantile(chains.LLL$lm,prob=c(0.025,0.975))))
...
rownames(inc.fit.jags.LLL) <- c("median",
                            "dispersion",
                            "p5","p25","p50","p75","p95")
...
require(knitr)
vshed.fit.jags.LLL <- exp(c(median(chains.LLL$V_lm), quantile(chains.LLL$V_lm,prob=c(0.025,0.975))))
...
rownames(vshed.fit.jags.LLL) <- c("median",
                            "dispersion",
                            "p5","p25","p50","p75","p95","p99")
...
**Table S8**: Lognormal distribution parameters and key quantiles of the distribution of time to seroconversion for Zika virus infection assuming all distributions are lognormal.


```{r, results='asis', echo=FALSE, message=FALSE}

require(knitr)
sero.fit.jags.LLL <- exp(c(median(chains.LLL$A_lm), quantile(chains.LLL$A_lm,prob=c(0.025,0.975))))
...
rownames(sero.fit.jags.LLL) <- c("median",
                            "dispersion",
                            "p5","p25","p50","p75","p95","p99")
...
**Table S9:**  Increase in risk of a Zika contaminated donation for each 1 per 100,000 
person-day increase in Zika incidence, assuming all lognormal distributions
```{r, results='asis', echo=FALSE}

#caclculate the virologic and symptom means and compare.
mns <- data.frame(inc = exp(chains.LLL$lm + chains.LLL$lsd^2/2),
...
Alternate methods of fitting the model and distributions and a spreadsheet comparing methods are available on the GitHub repository. Key quantiles were robust across all methods and assumptions, and the viral sheddind distribution was the most sensitive to the selection of the fitting approach.
